...
, SUBMIT_BY , ROLE_NAME , STATUS_TASK_MSS , APP_NO 
, NOTES  , pin.PROD_OFFERING_CODE 
from t_mkt_polo_order_in pin
join t_mkt_polo_order_in_x pinx on pin.t_mkt_polo_order_in_id=pinx.t_mkt_polo_order_in_id
where pin.TASK_ID ='POL000797573'
--AND pin.AGRMNT_NO ='1019120211110634'

 
OFFOFFMTRKUK

pin.TASK_ID ='POL000797517'

pin.ID_NO ='3511000901707212'

SELECT * FROM M_MKT_POLO_PARAMETER mmpp WHERE PARAMETER_ID LIKE '%PREAPPROVAL%'
...
select a.id_no,a.AGRMNT_ID,a.AGRMNT_NO
,a.CUST_NAME, a.BIRTH_DT , a.BIRTH_PLACE 
, a.SPOUSE_ID_NO , a.SPOUSE_NAME , a.SPOUSE_BIRTH_DT , a.SPOUSE_BIRTH_PLACE 
, a.MOTHER_NAME ,a.SPOUSE_MOTHER_NAME 
, a.MOBILE1 , a.SPOUSE_MOBILE_PHONE_NO 
, a.ZIPCODE_RES , a.SUB_ZIPCODE_RES 
, a.MARITAL_STAT  , a.CUST_RATING 
, a.CAE_FINAL_RESULT 
,tmpoi.TASK_ID ,tmpoix.PENGAJUAN_NO , tmpoi.ORDER_NO  
,tmpoix.IS_PRE_APPROVAL , tmpoi.FINAL_RESULT_CAE 
, a.birth_dt, a.birth_place
...
where a.is_active = 1
--and is_processed = 1
--and a.is_pre_approval = 1
--and a.office_name='BEKASI'
--and a.marital_stat='SIN'
--AND a.LOB_CODE LIKE '%MGJMTRKON%'
--AND a.CUST_RATING ='EXCELLENT'
--AND a.SUB_ZIPCODE_RES ='G5114'
--and a.contract_stat <>'LIV'
--and a.product_category ='MOTORKU' 
--and DATEDIFF(year, cast(a.BIRTH_DT as date ), cast(getdate() as date))  >34
--AND a.AGRMNT_NO ='1011120210207733 '
and a.id_no='3172021403710008'
--'3175076708740004''3201275810890005'--'3403032004840002'

SELECT  AGRMNT_ID, AGRMNT_NO, IS_ACTIVE FROM t_mkt_polo_eligible WHERE ID_NO ='3275036801930011'
...
SELECT * FROM confins.dbo.REF_EMP re WHERE re.EMP_NO ='08310763'
...
SELECT * FROM CONFINS.dbo.REF_USER ru WHERE ru.USERNAME ='286220001' OR USERNAME ='002170272'--002170272
...
DECLARE 
@JSON nvarchar(Max)
set @JSON = (	select top 1 LOG_PAYLOAD from WISE_STAGING..LOG_REQUEST_RESPONSE with(nolock) 
				where LOG_FUNCTION LIKE'%submitnc%' and LOG_TYPE = 'REQUEST' and CONVERT(DATE,LOG_DT) = '2024-08-01' and LOG_PAYLOAD like '%POL000797531%'
				order by LOG_DT desc
			 )
BEGIN
SELECT * from openjson(@JSON) 
WITH (
		"taskid" VARCHAR(100),"esignPermission" VARCHAR(100), "essignPasangan" VARCHAR(100)
	 )
END

--cek kiriman flag essign mss ke polo
DECLARE 
@JSON nvarchar(Max)
set @JSON = (	select top 1 LOG_PAYLOAD from WISE_STAGING..LOG_REQUEST_RESPONSE with(nolock) 
				where LOG_FUNCTION LIKE'%updatefrommss%' and LOG_TYPE = 'REQUEST' and CONVERT(DATE,LOG_DT) = '2024-07-23' and LOG_PAYLOAD like '%POL000797531%'
				order by LOG_DT desc
			 )
BEGIN
SELECT * from openjson(@JSON) 
WITH (
		"TaskId" VARCHAR(100),"OrderNo" VARCHAR(100), "StatusTaskMss" VARCHAR(100),  "AutoPresurvey" VARCHAR(100),  "IsDigitalSign" VARCHAR(100),
		 "IsSpouseDigitalSign" VARCHAR(100)
	 )
END
...
,LOG_PAYLOAD,LOG_FUNCTION , LOG_TYPE  ,LOG_METHOD ,  LOG_IP
from LOG_REQUEST_RESPONSE lrr 
where log_dt>'2024-08-01 14:00:25.670'
--and log_dt <='2023-12-22 15:10:53.077'
--and LOG_FUNCTION  like '%TASK%POLO%'
and LOG_PAYLOAD like '%POL000797550%'
...
,case when isjson(LOG_PAYLOAD)=1  then json_value(LOG_PAYLOAD, '$[0].NextStep.value') end NextStep 
from LOG_REQUEST_RESPONSE lrr 
where LOG_ID in ('0ead62d6-f00e-4ea9-b24b-45c2bbc5cbd3')
order by LOG_DT  desc
...
LEFT JOIN confins.dbo.agrmnt ag ON ap.APP_ID =ag.APP_ID 
LEFT JOIN confins.dbo.app_x apx ON ap.APP_ID =apx.APP_ID 
LEFT JOIN confins.dbo.cust cu ON cu.CUST_ID =ap.CUST_ID 
LEFT JOIN confins.dbo.CUST_PERSONAL cp ON cp.CUST_ID =ap.CUST_ID 
LEFT JOIN confins.dbo.CUST_PERSONAL_X cpx ON cp.CUST_ID =cpx.CUST_ID 
...
WHERE ap.APP_NO ='APP004120240600182'
...
WHERE lrr.LOG_FUNCTION LIKE '%POL000797083%' 
OR LOG_PAYLOAD LIKE '%POL000797083%'
ORDER BY log_dt desc
...
select top 10 lrr.LOG_ID  , LOG_FUNCTION , LOG_TYPE  ,LOG_METHOD ,  LOG_IP , LOG_DT
from LOG_REQUEST_RESPONSE lrr
where log_dt>'2024-08-01 10:00:36.823'
--and log_dt <='2023-12-18 18:14:18.673'
--and LOG_FUNCTION  like '%NEXT_TASK%'
and LOG_PAYLOAD like '%POL000797517%'
order by LOG_DT  desc

select * from LOG_REQUEST_RESPONSE lrr where LOG_ID  = 'bbb2ee70-7398-4a51-ba17-c1bf7a9e21c3' 
...
SELECT * FROM confins.dbo.ref_emp WHERE emp_name LIKE '%wibi%'
...
//asset
SELECT AAC.AGRMNT_ASSET_ID, 
					   AAC.ATTR_CONTENT, * 
				FROM ASSET_ATTR_CONTENT AAC WITH(NOLOCK) 
				JOIN AGRMNT_ASSET AAs WITH(NOLOCK) ON AAs.AGRMNT_ASSET_ID = AAC.AGRMNT_ASSET_ID
				JOIN ASSET_ATTR AATR WITH(NOLOCK) ON AATR.ASSET_ATTR_ID = AAC.ASSET_ATTR_ID 
				JOIN REF_ASSET_ATTR RAA WITH(NOLOCK) ON RAA.REF_ASSET_ATTR_ID = AATR.REF_ASSET_ATTR_ID 
				WHERE RAA.ATTR_CODE = 'COLOR' AND AAs.AGRMNT_ID = 7689065 
		...
		AA.SERIAL_NO_1, 
		AA.SERIAL_NO_2, 
		AA.MANUFACTURING_YEAR, 
		AA.LICENSE_PLATE_NO, 
		AM.ASSET_NAME, 
		HL1.ASSET_HIERARCHY_L1_NAME, 
		HL2.ASSET_HIERARCHY_L2_NAME, 
		AAR.OWNER_NAME, 
		AA.MR_ASSET_CONDITION 
		FROM AGRMNT_ASSET AA WITH(NOLOCK) 
		JOIN ASSET_MASTER AM WITH(NOLOCK) ON AA.ASSET_MASTER_ID = AM.ASSET_MASTER_ID 
		JOIN ASSET_CATEGORY AC WITH(NOLOCK) ON AM.ASSET_CATEGORY_ID = AC.ASSET_CATEGORY_ID 
		JOIN AGRMNT_ASSET_REGISTRATION AAR WITH(NOLOCK) ON AA.AGRMNT_ASSET_ID = AAR.AGRMNT_ASSET_ID 
		JOIN ASSET_HIERARCHY_L1 HL1 WITH(NOLOCK) ON AM.ASSET_HIERARCHY_L1_ID = HL1.ASSET_HIERARCHY_L1_ID 
		JOIN ASSET_HIERARCHY_L2 HL2 WITH(NOLOCK) ON AM.ASSET_HIERARCHY_L2_ID = HL2.ASSET_HIERARCHY_L2_ID 
		JOIN ASSET_TYPE AT WITH(NOLOCK) ON AC.ASSET_TYPE_ID = AT.ASSET_TYPE_ID 
		--WHERE AT.ASSET_TYPE_CODE = 'MOTOR'
		WHERE AA.AGRMNT_ID = 7689065
...
, vwt.START_EXECUTING_DATE , vwt.END_EXECUTING_DATE , vwt.DTM_CRT , vwt.USR_CRT , vwt.DTM_UPD ,vwt.USR_UPD 
--INTO #tempData
FROM confins.dbo.APP a 
LEFT JOIN confins.dbo.agrmnt ag ON ag.APP_ID = a.APP_ID 
JOIN confins.dbo.CUST_PERSONAL cp ON cp.CUST_ID =a.CUST_ID 
JOIN confins.dbo.CUST_PERSONAL_X cpx ON cpx.CUST_ID = a.CUST_ID 
JOIN confins.dbo.CUST c ON c.CUST_ID =a.CUST_ID 
...
and role_name like '%credit%'
--and rf.FORM_NAME  like '%credit%%'
and utr.IS_ACTIVE=1
and ru.IS_ACTIVE =1
and ro.office_name like '%CILEUNGSI%'--446170797
...
SELECT top 1000 b.UUID_TASK_DETAIL_LOB,A.DTM_CRT, DOWNLOAD_DATE, START_DTM, A.UUID_TASK_H, B.IMAGE_PATH, B.QUESTION_TEXT, B.QUESTION_ID, A.APPL_NO
FROM TR_TASK_H A with(nolock)
JOIN TR_TASKDETAILLOB B with(nolock) ON A.UUID_TASK_H = B.UUID_TASK_H
JOIN MS_STATUSTASK ms with(nolock) ON A.UUID_STATUS_TASK = ms.UUID_STATUS_TASK
--WHERE CAST (A.DTM_CRT AS DATE) = '2024-05-24'
AND A.APPL_NO = 'APP004120240600189'
--AND ms.status_task_desc = 'Reject with Re-Survey'
--AND B.IMAGE_PATH = '0'